...
func SaveArticleMetadata(conn *pgx.Conn, title, url, published, imageURL string) error {
    ...
    _, err := conn.Exec(
        ...
        published,
        markValue, 
        lastProcessedTime, // Langsung time.Time
        statusValue,
        imageURL, // Parameter baru
    )
